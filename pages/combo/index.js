...
import {fetchComboCategories} from "../../services/ComboCategoryServices";
...
const ComboPage = () => {
    ...
    const fetchCombosData = (params = {}) => {
        fetchCombos(params).then((response) => {
    ...
    return (
        <Fragment>
            <Head>
                <title>{makeTitle("Combo Products")}</title>
            </Head>
            <section>

                {/*Category Banner*/}
                {comboCategories?.[0]?.image && (
                    <div className="product-banner">
                        <img
                            // src={getStoragePath(`combo-category-image/${comboCategories?.[0]?.image}`)}
                ...
                <div className="container">
                    ...
                    <div className="row">
                        ...
                        <div className="col-lg-3 col-md-4 col-sm-4">
                            <ul className="stickyContent list-unstyled text-start ps-3 font-20 lh-lg card-border py-3 ">
                                {comboCategories?.map((item, key) => (
                                    <li key={key}>
                                        <Link href={`/combo/category/${item.id}`}>
                                            <button className="d-flex align-items-center category-btn">
